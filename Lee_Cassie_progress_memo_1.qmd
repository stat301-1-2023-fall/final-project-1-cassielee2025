...
I am able to read in each data file. In total, I have 135 variable across all the datasets, but each dataset contains several identifiers such as `state_fips`, `state`, `county_fips`, `county`, and `year`. There are also several variables throughout the datasets such as  `data_comment`, `x8`, `x11`, or 95% confidence intervals are completely missing or are not relevant to use. 
